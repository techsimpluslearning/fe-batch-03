          ...
          currentTodos.map((v,i)=>{
            return <div className="col-4 mb-4">
              <div className="card p-4 shadow">
                <h3>{v.title}</h3>
                <p>{v.description}</p>
                <span className={` badge ${v.priority === "High"? "bg-danger": v.priority === "Medium"? "bg-warning": "bg-success"}`}> {v.priority}</span>
        ...
        <div className="d-flex gap-3 justify-content-center">
          {
            activePaginList.map((n)=>{
              return <span onClick={() => setCurrentPage(n)} className={`dot p-2 border rounded ${ currentPage === n ? "bg-dark text-white" : "bg-light"}`}style={{ cursor: "pointer" }}>
                {n}
              </span>
            })
          }
...
//       <div className="row mt-4">
//         <div className="d-flex gap-3 justify-content-center">
//           {activePaginList?.map((num) => (
//             <span
//               key={num}
//               onClick={() => setCurrentPage(num)}
//               className={`dot p-2 border rounded ${
//                 currentPage === num ? "bg-dark text-white" : "bg-light"
//               }`}
